...
export default function DashboardPage() {
  ...
  // Si el usuario es seller, filtramos siempre por su sucursal
  const isAdmin = session?.role === "admin";
  const isSeller = session?.role === "seller";
  const locationId = isSeller ? (session?.locationId ?? null) : null;
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      const [advMetrics, chartData, topData, salesList, alerts] = await Promise.all([
        dashboardService.getAdvancedMetrics(effectiveTenantId, locationId),
        dashboardService.getWeeklySalesData(effectiveTenantId, locationId),
        dashboardService.getTopProductsData(effectiveTenantId, 5, locationId),
  ...
  const dashboardSubtitle = isSeller
    ? "Resumen de tu sucursal — ventas y stock de hoy"
        ...
        <div className="p-4 bg-white rounded-xl border border-[#DDD9D0] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-[#EBF0EC] text-[#556B5D] rounded-xl">
              {isSeller ? <Store className="w-5 h-5" /> : <ShieldCheck className="w-5 h-5" />}
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                {isSeller && (
                  <span className="text-xs text-[#6B7A71]">
                    Datos exclusivos de tu punto de venta
                  </span>
                )}
                {isAdmin && (
                  <span className="text-xs text-[#6B7A71] hidden sm:inline">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card padding="md" className="border-l-4 border-l-[#C49A5A]">
                ...
                <div className="mt-2">
                  <span className="text-2xl font-bold text-[#26302B] font-[Outfit]">
                    {loading ? "..." : formatCurrency(metrics.revenueThisWeek)}
                  </span>
                  <span className="block text-[11px] text-[#6B7A71] font-medium mt-0.5">
                    {loading ? "..." : `${metrics.salesThisWeek} ventas en 7 días`}
                  </span>
                </div>
              </Card>
              ...
              <Card padding="md" className="border-l-4 border-l-[#8FA393]">
                ...
                <div className="mt-2">
                  <span className="text-2xl font-bold text-[#26302B] font-[Outfit]">
                    {loading ? "..." : metrics.totalInventoryUnits.toLocaleString()}
                  </span>
                  <span className="block text-[11px] text-[#6B7A71] font-medium mt-0.5">
                    piezas disponibles en tu local
                  </span>
                </div>
              </Card>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-sm font-bold text-[#26302B] flex items-center gap-2">
                      ...
                      Tendencia Semanal — Tu Sucursal
                    </h3>
                    <p className="text-xs text-[#6B7A71] mt-0.5">
                      Ventas de los últimos 7 días en tu punto de venta
                    </p>
                  </div>
                  <Badge variant="success">Tu Local</Badge>
                </div>
                {loading ? (
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-sm font-bold text-[#26302B] flex items-center gap-2">
                      ...
                      Top Vendidos — Tu Sucursal
                    </h3>
                    <p className="text-xs text-[#6B7A71] mt-0.5">
                      Modelos con mayor rotación en tu local
                    </p>
                  </div>
                </div>
                {loading ? (
            ...
            {/* Accesos Rápidos del Vendedor */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <Link href="/pos" className="group">
                <div className="p-4 bg-[#26302B] text-white rounded-xl flex flex-col items-center gap-2 text-center hover:bg-[#3a4d43] transition-colors cursor-pointer">
                  <ShoppingCart className="w-6 h-6" />
                  <span className="text-xs font-bold">Punto de Venta</span>
                </div>
              </Link>
              <Link href="/inventario" className="group">
                <div className="p-4 bg-white border border-[#DDD9D0] text-[#26302B] rounded-xl flex flex-col items-center gap-2 text-center hover:bg-[#F8F6F1] transition-colors cursor-pointer">
                  <Boxes className="w-6 h-6 text-[#556B5D]" />
                  <span className="text-xs font-bold">Mi Inventario</span>
                </div>
              </Link>
              <Link href="/ventas" className="group">
                <div className="p-4 bg-white border border-[#DDD9D0] text-[#26302B] rounded-xl flex flex-col items-center gap-2 text-center hover:bg-[#F8F6F1] transition-colors cursor-pointer">
                  <Receipt className="w-6 h-6 text-[#556B5D]" />
                  <span className="text-xs font-bold">Mis Ventas</span>
                </div>
              </Link>
              <Link href="/clientes" className="group">
                <div className="p-4 bg-white border border-[#DDD9D0] text-[#26302B] rounded-xl flex flex-col items-center gap-2 text-center hover:bg-[#F8F6F1] transition-colors cursor-pointer">
                  <PackagePlus className="w-6 h-6 text-[#556B5D]" />
                  <span className="text-xs font-bold">Clientes</span>
                </div>
              </Link>
            </div>

            {/* Últimas Ventas + Alertas de la sucursal */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Últimas Ventas */}
              <Card padding="md">
                <div className="flex items-center justify-between mb-3">
                  <CardHeader
                    title="Últimas Ventas — Tu Sucursal"
                    subtitle="Historial reciente de tu punto de venta"
                  />
                  <Link
                    href="/ventas"
                ...
                {loading ? (
                  ...
                  <div className="p-8 text-center border-2 border-dashed border-[#E7E3DA] rounded-lg">
                    <p className="text-xs text-[#6B7A71]">
                      Aún no hay ventas. Realice la primera venta desde el POS.
                    </p>
                  </div>
                ) : (
              ...
              <Card padding="md">
                <div className="flex items-center justify-between mb-3">
                  <CardHeader
                    title="Alertas de Stock — Tu Tienda"
                    subtitle="Prendas agotadas o cerca del mínimo en tu local"
                  />
                  <Link
                    href="/inventario"
                    className="text-xs font-semibold text-[#556B5D] hover:underline flex items-center gap-1"
                ...
                {loading ? (
                  ...
                  <div className="p-8 text-center border-2 border-dashed border-[#E7E3DA] rounded-lg">
                    <Badge variant="success">Stock Saludable</Badge>
                    <p className="text-xs text-[#6B7A71] mt-2">
                      No hay alertas activas en tu local.
                    </p>
                  </div>
                ) : (
                  <div className="divide-y divide-[#DDD9D0] border border-[#DDD9D0] rounded-xl overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card padding="md" className="border-l-4 border-l-[#C49A5A]">
                ...
                <div className="mt-2">
                  ...
                  <span className="block text-[11px] text-[#6B7A71] font-medium mt-0.5">
                    Acumulado mensual activo — todas las sucursales
                  </span>
                </div>
              </Card>
              ...
              <Card padding="md" className="lg:col-span-2">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-[#6B7A71] mt-0.5">
                      Ventas brutas acumuladas — todas las sucursales
                    </p>
                  </div>

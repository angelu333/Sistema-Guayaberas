        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#DDD9D0] bg-[#F8F6F1] print:hidden">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7A71]">Vista previa del ticket / documento formal de presupuesto</p>
            </div>
          </div>
        ...
        >
          ...
          <div className="flex justify-between items-start border-b-2 border-[#556B5D] pb-6">
            <div className="flex items-center gap-4">
              {quote.tenantInfo?.logoUrl ? (
                <img
                  src={quote.tenantInfo.logoUrl}
                  alt="Logo"
                  className="w-16 h-16 object-contain rounded-xl border border-[#DDD9D0]"
                />
              ) : (
                <div className="w-14 h-14 rounded-2xl bg-[#26302B] text-white flex items-center justify-center font-bold font-[Outfit] text-2xl shadow-md border-2 border-[#C49A5A]">
                  G
                </div>
              )}
              <div>
                <h1 className="font-[Outfit] text-2xl font-extrabold text-[#26302B] tracking-tight">
                  {quote.tenantInfo?.name || "Guayabera Manager"}
                </h1>
                <p className="text-xs font-semibold text-[#556B5D]">Confección & Mayoreo de Guayaberas Finas</p>
                {quote.tenantInfo?.phone && (
                  <p className="text-xs text-[#6B7A71] mt-0.5">Teléfono: {quote.tenantInfo.phone}</p>
                )}
            ...
            <div className="text-right">
              <span className="inline-block px-3.5 py-1 bg-[#26302B] text-white font-mono font-bold text-sm rounded-lg shadow-xs">
                {quote.quoteNumber}
              </span>
              <p className="text-xs text-[#6B7A71] mt-2">
                Fecha de Emisión: <strong>{new Date(quote.createdAt).toLocaleDateString("es-MX")}</strong>
              </p>
          ...
          <div className="p-4 bg-[#F8F6F1] rounded-2xl border border-[#DDD9D0] grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm font-extrabold text-[#26302B] mt-0.5">{quote.clientName}</p>
              {quote.clientPhone && (
                <p className="text-xs text-[#6B7A71]">WhatsApp / Tel: {quote.clientPhone}</p>
              )}
            </div>
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-extrabold text-[#3F7D58] mt-0.5">
                {quote.totalPieces} guayaberas
              </p>
              {quote.discountAmount > 0 && (
                <span className="inline-block text-[11px] font-bold text-[#3F7D58] bg-[#EBF5F0] px-2.5 py-0.5 rounded-md mt-1 border border-[#A7D7B9]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b-2 border-[#26302B] bg-[#F8F6F1] text-[11px] font-extrabold text-[#26302B] uppercase tracking-wider">
                <th className="py-3 px-3">Modelo / Guayabera</th>
                <th className="py-3 px-3 text-center">Detalle / Talla</th>
                <th className="py-3 px-3 text-center">Cant.</th>
                <th className="py-3 px-3 text-right">Precio Reg.</th>
                <th className="py-3 px-3 text-right">P. Mayoreo</th>
                <th className="py-3 px-3 text-right">Subtotal</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#DDD9D0] text-xs">
              {quote.details.map((d) => (
                <tr key={d.id} className="hover:bg-[#F8F6F1]/50">
                  <td className="py-3 px-3 font-bold text-[#26302B]">
                    {d.productName}
                    <span className="block text-[10px] text-[#6B7A71] font-mono font-normal">
                      SKU: {d.sku}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-3 text-center font-bold text-[#26302B]">{d.quantity} pzas</td>
          ...
          <div className="flex justify-end pt-4 border-t border-[#DDD9D0]">
            <div className="w-72 space-y-2 text-xs">
              <div className="flex justify-between text-[#6B7A71]">
                <span>Subtotal Regular ({quote.totalPieces} pzas):</span>
                <span className="font-mono">${quote.subtotal.toFixed(2)}</span>
              </div>

              {quote.discountAmount > 0 && (
                <div className="flex justify-between text-[#3F7D58] font-bold bg-[#EBF5F0] p-2 rounded-lg border border-[#A7D7B9]">
                  <span>Ahorro Total por Mayoreo:</span>
                  <span className="font-mono">-${quote.discountAmount.toFixed(2)}</span>
                </div>
              )}

              <div className="flex justify-between text-base font-extrabold text-[#26302B] pt-2 border-t-2 border-[#26302B]">
                <span>TOTAL ESTIMADO:</span>
                <span className="font-mono text-[#3F7D58] text-lg">${quote.totalAmount.toFixed(2)} MXN</span>
              </div>
            </div>
          </div>
